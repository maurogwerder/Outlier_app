# Modified version of function 'extract_features' from package 'tsfeaturex'
# Original author: Nelson Roque

# Edits: default value of arg 'custom_feature_list' waschanged from NA to NULL, to
# enable checking lists instead of only single values
...
#' @import entropy
#' @import zoo
extract_features_edit <- function(df, group_var, value_var, features='all', custom_feature_list=NULL, data_confirm=T, return_timing=T, verbose=T){
  ...
  if(!data_confirm){
    ...
    if(features == "custom") {
      if(is.null(custom_feature_list)){
...
#' @export
features_to_df_edit <- function(df.list, group_var, data.format = "long", verbose=F) {
  VERSION_CODE = packageVersion("tsfeaturex")
